...
class DataModel(object):
    ...
    def init_table(cls):
        ...
        cls._cursor.execute("CREATE TABLE IF NOT EXISTS migration_metadata(id SERIAL PRIMARY KEY, type VARCHAR(16), name VARCHAR(32), sql VARCHAR(1024));")
        ...
        if row is not None and row[0] != sql:
            ...
            # Add table metadata
            cls._cursor.execute("INSERT INTO migration_metadata(type, name, sql) VALUES (%s, %s, %s);", ("TABLE", cls.table, sql))
            cls._cursor.execute("COMMIT;")
